...
import static org.apache.seata.discovery.loadbalance.LoadBalanceFactory.TXG_LOAD_BALANCE;


/**
 * The type TXG load balance.
 *
 */
@LoadLevel(name = TXG_LOAD_BALANCE)
public class TXGLoadBalance implements LoadBalance {
    ...
    public <T> T select(List<T> invokers, String xid) throws Exception {
        // The xid parameter is used to pass the TXG when this load balancer is selected
        // In AbstractNettyRemotingClient, getTXG() should be called and passed as the xid parameter
        if (StringUtils.isNotBlank(xid) && xid.contains(SPLIT)) {
            ...
            InetSocketAddress txgInetSocketAddress = new InetSocketAddress(ip, port);
            for (T invoker : invokers) {
                InetSocketAddress inetSocketAddress = (InetSocketAddress) invoker;
                if (Objects.equals(txgInetSocketAddress, inetSocketAddress)) {
                    return (T) inetSocketAddress;
                }
            }
            LOGGER.error("not found seata-server channel, txg: {}, try use random load balance", xid);
        }
